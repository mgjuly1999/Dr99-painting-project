      ...
      <div class="row">
        ...
        <div class="card-body">
			...
			<th>Product Detail</th>
                ...
                <tbody>
                <c:forEach var="p" items="${showAllProductForm.showProductForms}" varStatus="s">
			<tr>
				<td>${s.index+1}</td>
				<td>${p.product.productId}</td>
				<td>${p.productTitleLanguage.name}</td>
				<td>${p.product.totalQty}${p.qtyUnit.unit}</td>
				<td>${p.typeLanguage.name}</td>
				<td>${p.groupLanguage.name}</td>
			    <th>
			    <a href="productDetilsPath?productId=${product.productId}" 
			    class="btn btn-block btn-outline-primary btn-sm">Product Detail</a></th>
