        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Show All Order Form</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
      ...
      <div class="row">
        ...
        <div class="col-10">
           <div class="card card-success">
                ...
                <h3 class="card-title">Show All Order Form </h3>
              </div>
        <div class="card-body">
			...
			<th>ShowDetail</th>
                ...
                <tbody>
                <c:forEach var="at" items="${orderFormRegisterForm.showOrderForms}" varStatus="s">
			<tr>
				<td>${s.index+1}</td>
				<td>${at.orderForm.orderFormId}</td>
			    <td>${at.orderForm.phone}</td>
			    <td>${at.orderForm.addressDetails}</td>
			    <td>${at.orderForm.totalDicountRate}</td>
			    <td>${at.orderForm.totalPrice}</td>
			    <td>${at.orderForm.note}</td>
			    <td>${at.orderStatus.statusName}</td>
			    <td>${at.deliveryType.name}</td>
			    <td>${at.paymentMethod.serviceName}</td>
			    <td>${at.deliveryAddress.address}</td>
			    <th><a href="orderFormUpdatePath?frmOrderFormId=${at.orderForm.orderFormId}" class="btn btn-block btn-outline-primary btn-sm">Update</a></th>
				<td><a href="showAllProductForOrderForm?frmOrderFormId=${at.orderForm.orderFormId}" class="btn btn-block btn-outline-primary btn-sm">ShowDetail</a></td>
			</tr>		
		    </c:forEach>
